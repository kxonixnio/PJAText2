...
auto print_words_sorted_alphabetically(
...
auto print_words_sorted_by_length(
        std::vector<std::string> file_content_word_by_word) -> void {    //kopia jest potrzebna, bo nie chce zmieniać oryginału
    std::sort(file_content_word_by_word.begin(), file_content_word_by_word.end(),
...
s::s(const std::vector<std::string> &content)
        : file_content(content), flag_name(std::string("-s")), flag_alias(std::string("--sorted")) {}

void s::do_stuff() const {
    if (!get_is_s_or_rs_modified()) {
        print_words_sorted_alphabetically(file_content);
    } else {
        print_words_sorted_by_length(file_content);
    }
}

const std::string &s::get_flag_name() const {
    return flag_name;
}

const std::string &s::get_flag_alias() const {
    return flag_alias;
}